...
public class CompoundButton extends android.widget.CompoundButton {
    ...
    private void initWithAttributes(Context context, AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CompoundButton);
        final int N = a.getIndexCount();
        for (int i = 0; i < N; i++) {
            int attr = a.getIndex(i);
            if (attr == R.styleable.CompoundButton_customFontFile) {
                setCustomFontFile(a.getString(attr));
            } else if (attr == R.styleable.CompoundButton_uppercase) {
